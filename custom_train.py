...
from AdabinsOriginal import model_io
#from AdabinsOriginal import models
from panoptic_adabins_combined import PanopticAdabinsCombined
from AdabinsOriginal import utils
#from AdabinsOriginal.dataloader import DepthDataLoader
from panoptic_adabins_dataloader import PanopticAdabinsDepthDataLoader
from AdabinsOriginal.loss import SILogLoss
from no_pytorch3d_loss import BinsChamferLoss
from AdabinsOriginal.utils import RunningAverage, colorize
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = models.UnetAdaptiveBins.build(n_bins=args.n_bins, min_val=args.min_depth, max_val=args.max_depth,
                                          norm=args.norm)
    """
    model = PanopticAdabinsCombined.build(n_bins=args.n_bins, min_val=args.min_depth, max_val=args.max_depth,
                                          norm=args.norm)
    ################################################################################################
...
def train(model, args, epochs=10, experiment_name="DeepLab", lr=0.0001, root=".", device=None,
    ...
    train_loader = PanopticAdabinsDepthDataLoader(args, 'train').data
    test_loader = PanopticAdabinsDepthDataLoader(args, 'online_eval').data
    ...
    for epoch in range(args.epoch, epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Train",
            ...
            #print(batch['name'])
            img_detectron = batch['img_detectron'].to(device)
            depth = batch['depth'].to(device)
            if 'has_valid_depth' in batch:
                if not batch['has_valid_depth']:
                    continue

            bin_edges, pred = model((img, img_detectron))
...
def validate(args, model, test_loader, criterion_ueff, epoch, epochs, device='cpu'):
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc=f"Epoch: {epoch + 1}/{epochs}. Loop: Validation") if is_rank_zero(
            ...
            bins, pred = model((img, img_detectron))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filenames_file',
                        default="./data/NYUv2-raw/train_test_subset/nyudepthv2_train_files_with_gt.txt",
    ...
    parser.add_argument('--data_path_eval',
                        default="./data/NYUv2-raw/official_splits/test/",
                        type=str, help='path to the data for online evaluation')
    parser.add_argument('--gt_path_eval', default="./data/NYUv2-raw/official_splits/test/",
                        type=str, help='path to the groundtruth data for online evaluation')
    parser.add_argument('--filenames_file_eval',
                        default="./data/NYUv2-raw/train_test_subset/nyudepthv2_test_files_with_gt.txt",
